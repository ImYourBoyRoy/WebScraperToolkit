...
from web_scraper_toolkit.browser._playwright_handler.constants import classify_bot_block  # noqa: E402
...
async def run_stage(plan: StagePlan, url_name: str, url: str) -> None:
    ...
    try:
        async with PlaywrightManager(plan.config) as manager:
            content, final_url, status = await manager.smart_fetch(
                url, action_name=plan.name
            )
            ...
            block_reason = classify_bot_block(
                status=status, final_url=final_url, content_html=content
            )

            print(
                f"Status: {status} | Elapsed: {elapsed:.1f}s | Content Len: {content_len}"
            )
