...
with patch('web_scraper_toolkit.server.mcp_server.read_website_markdown'), \
...
class TestMCPServer(unittest.TestCase):
    ...
    def test_tool_methods_call_wrapper(self, mock_run):
        ...
        # 3. Sitemap
        res = self.loop.run_until_complete(mcp_server.get_sitemap("https://example.com"))
        args, _ = mock_run.call_args
        self.assertEqual(args[0].__name__, "get_sitemap_urls")
        
        # 4. Screenshot
        res = self.loop.run_until_complete(mcp_server.screenshot("https://example.com", "path.png"))
        args, _ = mock_run.call_args
