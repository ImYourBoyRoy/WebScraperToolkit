...
async def _arun_scrape(website_url: str, config: Optional[Union[Dict[str, Any], ScraperConfig]] = None) -> str:
    ...
    try:
        ...
        if status_code == 200 and content:
            ...
            leadership_keywords = ['CEO', 'Chief Executive', 'Founder', 'President', 'Owner', 'Director']
            ...
            logger.info(f"Successfully scraped and structured {len(main_content)} characters from {final_url}")
            return output
        else:
            return f"Error: Failed to retrieve content from {website_url}. Status code: {status_code}"
    except Exception as e:
        logger.error(f"An error occurred while scraping {website_url}: {e}", exc_info=True)
        return f"An error occurred while scraping the website: {str(e)}"
    finally:
        if manager:
            await manager.stop()

def read_website_content(website_url: str, config: Optional[Union[Dict[str, Any], ScraperConfig]] = None) -> str:
    """
    ...
    try:
        ...
        if loop.is_running():
            future = asyncio.run_coroutine_threadsafe(_arun_scrape(website_url, config), loop)
...
async def _arun_scrape_markdown(website_url: str, config: Optional[Union[Dict[str, Any], ScraperConfig]] = None) -> str:
    ...
    try:
        ...
        if status_code == 200 and content:
            ...
            logger.info(f"Successfully scraped and converted {len(markdown)} chars from {final_url}")
            return output
        else:
            return f"Error: Failed to retrieve content from {website_url}. Status code: {status_code}"
    except Exception as e:
...
def read_website_markdown(website_url: str, config: Optional[Union[Dict[str, Any], ScraperConfig]] = None) -> str:
    """
    ...
    try:
        loop = asyncio.get_running_loop()
        if loop.is_running():
            future = asyncio.run_coroutine_threadsafe(_arun_scrape_markdown(website_url, config), loop)
            return future.result()
        else:
...
def extract_metadata(website_url: str, config: Optional[Union[Dict[str, Any], ScraperConfig]] = None) -> str:
    """
    ...
    try:
        ...
        if loop.is_running():
             ...
             future = asyncio.run_coroutine_threadsafe(_arun_extract_metadata(website_url, config), loop)
             return future.result()
        else:
...
async def _arun_extract_metadata(website_url: str, config: Optional[Union[Dict[str, Any], ScraperConfig]] = None) -> str:
    from ..browser.playwright_handler import PlaywrightManager
    manager = PlaywrightManager(config=config)
    await manager.start()
    try:
        ...
        for meta in soup.find_all('meta'):
            ...
            if name and content:
                if any(x in name for x in ['og:', 'twitter:', 'description', 'keywords', 'author']):
...
def capture_screenshot(website_url: str, output_path: str, config: Optional[Union[Dict[str, Any], ScraperConfig]] = None) -> str:
...
async def _arun_screenshot(url: str, path: str, config: Optional[Union[Dict[str, Any], ScraperConfig]] = None):
    config = config or {}
    from ..browser.playwright_handler import PlaywrightManager
...
def save_as_pdf(website_url: str, output_path: str, config: Optional[Union[Dict[str, Any], ScraperConfig]] = None) -> str:
...
async def _arun_pdf(url: str, path: str, config: Optional[Union[Dict[str, Any], ScraperConfig]] = None):
    config = config or {}
    # Force headless for PDF
    if "scraper_settings" not in config: config["scraper_settings"] = {}
...
async def _arun_search(search_query: str, config: Optional[Union[Dict[str, Any], ScraperConfig]] = None) -> str:
    ...
    try:
        ...
        if "CEO" in search_query or "leadership" in search_query:
            enhanced_query = f'{search_query} site:linkedin.com' # Simplified site search for DDG
        ...
        priority_domains = ['linkedin.com', 'bloomberg.com', 'crunchbase.com', 'reuters.com', 'wsj.com']
...
def general_web_search(search_query: str, config: Optional[Union[Dict[str, Any], ScraperConfig]] = None) -> str:
    """
    ...
    try:
        loop = asyncio.get_running_loop()
        if loop.is_running():
            future = asyncio.run_coroutine_threadsafe(_arun_search(search_query, config), loop)
            return future.result()
        else:
...
def get_sitemap_urls(*args, **kwargs) -> str:
    ...
    try:
        ...
        if root.tag == f'{namespace}sitemapindex':
            sitemap_links = [elem.text for elem in root.findall(f".//{namespace}loc") if elem.text]
            for link in sitemap_links:
                try:
                    nested_response = requests.get(link, timeout=10, headers={'User-Agent': 'Mozilla/5.0'})
                    nested_response.raise_for_status()
                    nested_root = ET.fromstring(nested_response.content)
                    urls = [elem.text for elem in nested_root.findall(f".//{namespace}loc") if elem.text]
                    all_urls.extend(urls)
                except requests.RequestException as e:
                    logger.warning(f"Could not fetch or parse nested sitemap {link}: {e}")
                    continue
        ...
        priority_keywords = ['about', 'contact', 'team', 'leadership', 'management', 'careers', 'news']
        exclude_keywords = [
            # E-commerce
            '/product/', '/cart/', '/checkout/', '/order-',
            # User Accounts
            '/my-account/', '/login/', '/log-in', '/register', '/registration', '/profile', '/dashboard', '/password',
            # CMS & Archives
            '/tag/', '/category/', '/author/', '/wp-content/', '/uploads/', '/page/', '?s=',
            # Learning Management Systems (LMS)
            '/courses/', '/lessons/', '/topic/', '/quiz/', '/certificates/', 'sfwd-', '/ldgr_group_code/',
            # Forums / Groups
            '/groups/',
            # Forms & Procedural
            '/inquiry', '/form/',
...
async def _arun_deep_research(search_query: str, config: Optional[Union[Dict[str, Any], ScraperConfig]] = None) -> str:
    ...
    try:
        ...
        content_html, final_url, status_code = await manager.fetch_page_content(page=page, url=search_url)
        await page.close()
        ...
        for i, url_to_crawl in enumerate(urls_to_crawl):
            ...
            page_content, _, _ = await manager.fetch_page_content(page=page, url=url_to_crawl)
...
def deep_research_with_google(search_query: str, config: Optional[Union[Dict[str, Any], ScraperConfig]] = None) -> str:
    """
    ...
    try:
        loop = asyncio.get_running_loop()
        if loop.is_running():
            future = asyncio.run_coroutine_threadsafe(_arun_deep_research(search_query, config), loop)
            return future.result()
        else:
...
def finish_research_for_field(field_path: str, reasoning: str) -> str:
    ...
    """
    log_message = f"Agent has concluded research for field '{field_path}'. Reason: {reasoning}"
